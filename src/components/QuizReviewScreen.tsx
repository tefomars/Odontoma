      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-10 grid gap-5">
          {attempt.responses.map((response, index) => (
            <article key={`${response.questionId}-${index}`} className={`rounded-[2rem] border p-6 ${response.isCorrect ? "border-emerald-500/30 bg-emerald-500/5" : "border-red-500/30 bg-red-500/5"}`}>
              <div className="flex items-start justify-between gap-4">
                <p className="text-xs font-black uppercase tracking-[0.2em] text-zinc-500">Pregunta {index + 1}</p>
                <span className={`rounded-full px-3 py-1 text-xs font-black ${response.isCorrect ? "bg-emerald-500/15 text-emerald-300" : "bg-red-500/15 text-red-300"}`}>{response.isCorrect ? "Correcta" : "Incorrecta"}</span>
              </div>
              <h2 className="mt-4 text-2xl font-black leading-tight">{response.question}</h2>
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <AnswerBox title="Tu respuesta" answers={response.selectedAnswers} tone={response.isCorrect ? "correct" : "wrong"} />
                <AnswerBox title="Respuesta correcta" answers={response.correctAnswers} tone="correct" />
              </div>
...
function AnswerBox({ title, answers, tone }: { title: string; answers: string[]; tone: "correct" | "wrong" }) {
  return (
    <div className={`rounded-2xl border p-4 ${tone === "correct" ? "border-emerald-500/25 bg-emerald-500/10" : "border-red-500/25 bg-red-500/10"}`}>
